...
/**
 * @author 刘镇 (suninformation@163.com) on 16/6/26 下午11:36
 * @version 1.0
 */
public class WxPayRefundQueryResponse extends WxPayBaseResponse {
    ...
    public static WxPayRefundQueryResponse bind(String protocol) throws Exception {
        return new WxPayRefundQueryResponse(protocol);
    }

    public WxPayRefundQueryResponse(String protocol) throws Exception {
        super(protocol);
        ...
        for (int i = 0; i < this.refundCount; i++) {
            ...
            _data.setRefundRecvAccount(BlurObject.bind(this.getResponseParams().get("refund_recv_accout_" + i)).toStringValue());
            //
